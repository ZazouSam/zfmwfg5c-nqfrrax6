...
export function TreemapChart({ data, colors }: { data: TreemapRootNode; colors: string[] }) {
  const rawData = data.children.flatMap((d) => d.children);
...
type TreemapRootNode = {
  name: string;
  children: TreemapBranchNode[];
};

export function DemoTreemapChart() {
  const rawData = [
  ...
  return <TreemapChart data={data} colors={colors} />;
}